...
def build_sample_weight(y,label2id,phone_level=True):
    ...
    sample_weight[ y_id == label2id['pad']] = 0.
    if phone_level:
...
def Mygenerator2(wavfiles,mfcc_feats,utt2LabelSeq,label2id,id2label,bucket_size,batch_size=128):
    ...
    while 1:
        ...
        for n in range(len(wavfiles)):
            f = os.path.basename(wavfiles[n])[:-4]
            mfcc = mfcc_feats[n][OFFSET:]
            labelseq = utt2LabelSeq[f][OFFSET:]
            utt_len = mfcc.shape[0]
            utt_bucket_n = int(math.ceil(utt_len / float(bucket_size)))

            x = np.lib.pad(mfcc,((0,utt_bucket_n*bucket_size-utt_len),\
                    (0,0)),'constant',constant_values=0.)
            y = np.lib.pad(labelseq,((0,utt_bucket_n*bucket_size-utt_len),\
                    (0,0)),'constant',constant_values=0.)
            if phone_level:
                y[utt_len:,label2id['<s>']] = 1.
            else:
...
def Mygenerator2_mfcc(wavfiles,mfcc_feats,utt2LabelSeq,label2id,id2label,bucket_size,batch_size=128):
    X_train = np.array([])
    Y_train = np.array([])
    OFFSET = 0
    while 1:
...
def ha():
    ...
    if os.path.isfile(join(weight_dir,weight_name)):
        ...
        if 'n' in stdin:
            sys.exit(1)
        stdin = raw_input('continue training?(y,n)')
        if 'y' in stdin:
            CONTINUE = True
    ...
    for epoch in range(nb_epoch):
        ...
        for bs in range(len(buckets)):
            ...
            for t,n_t in zip(targets,range(len(targets))):
                ...
                mlfpath = join(join(target_prefix,t), mlf_suffix)
                dicpath = join(join(target_prefix,t), dic_suffix)
                utt2LabelSeq,label2id,id2label = util.MLFReader(mlfpath,cfgpath,dicpath, phone_level = phone_level) ### load y target
                ...
                history = model.fit_generator(Mygenerator2(wavfiles,mfcc_feats,utt2LabelSeq,\
                                            ...
                                            samples_per_epoch=160000, nb_epoch=1)
                bucket_loss += history.history['loss'][-1]
            bucket_loss /= len(targets)
            #bucket_loss /= len(buckets)
            ...
            #        format(epoch,t,bucket_loss,time.time() - t_start_time))

        print ('Epoch {:3d} loss: {:.5f} fininish in: {:.5f} sec !!!!!!'\
                .format(epoch,epoch_loss/len(buckets),time.time() - start_time))
        #print ('Epoch {:3d} loss: {:.5f} fininish in: {:.5f} sec !!!!!!'\
        #        .format(epoch,epoch_loss/len(targets),time.time() - start_time))
...
def ha_mfcc():
    ...
    buckets = [40]
    nb_epoch = 1
    weight_name = 'fixed40_nosli_mfcc.h'
    #weight_name = 'bucketv2_nosli_mfcc.h'
    ...
    for epoch in range(nb_epoch):
        ...
        for bs in range(len(buckets)):
            ...
            checkpointer = ModelCheckpoint(filepath=join(weight_dir,weight_name),monitor='loss',\
                    verbose=1, save_best_only=True,save_weights_only=True)

            history = model.fit_generator(Mygenerator2_mfcc(wavfiles,mfcc_feats,utt2LabelSeq,\
                                        label2id,id2label,buckets[bs]),\
                                        samples_per_epoch=40000, nb_epoch=1000,callbacks=[checkpointer])
...
def test(): 
    ...
    weight_dir = 'model_weights'
    weight_name = 'bucketv2_fuck_tmp.h'
    dumpfile = 'bucketv2_fuck_word_align.p'
    ...
    load_word_align(word_align_path,word_aligns)
    query_aligns = {}
    load_word_align(test_set,query_aligns)
    ...
    for n,wav in enumerate(wavfiles):
        wav = os.path.basename(wav)[:-4] # remove '.wav'
        if wav not in query_aligns:
            continue
        for word in query_aligns[wav]:
    ...
    with open(dumpfile,'w') as f:
        cPickle.dump(query_aligns, f, protocol=cPickle.HIGHEST_PROTOCOL)
...
def test_mfcc():
    ...
    weight_dir = 'model_weights'
    fix_len = 40
    weight_name = 'fixed40_nosli_mfcc_tmp.h'
    dumpfile = 'fixed40_word_align_mfcc.p'
    ...
    for n,wav in enumerate(wavfiles):
        ...
        for word in query_aligns[wav]:
            ...
            if word.length > 80:
                ...
                for length in buckets:
                    if length > word.length:
                        ...
                        break
            ### build model
            model = Seq2Seq(input_shape=(b_size,39), output_dim=39,output_length=b_size,hidden_dim=hid_dim)
            model.compile(loss='mse', optimizer = 'adadelta',sample_weight_mode="temporal")
            model.load_weights(join(weight_dir,weight_name))
            get_mid_layer_output = K.function([model.layers[0].input], [model.layers[-2].input])
            ...
            X = GetTestX(n,wavfiles,mfcc_feats,word,(b_size,b_size))
            '''
            X = GetTestX(n,wavfiles,mfcc_feats,word,(fix_len,fix_len))
...
def seq2mseq_map():
    with open('bucketv2_fuck_word_align.p','r') as f:
...
def mfcc2mfcc_map():
    with open('fixed40_word_align_mfcc.p','r') as f:
        words = cPickle.load(f)
